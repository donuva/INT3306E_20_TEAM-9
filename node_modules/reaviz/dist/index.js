...
import { Component, PureComponent, Fragment, createRef, useMemo, Children, cloneElement, createContext, useContext, useState, useCallback, useEffect, useRef, isValidElement } from "react";
...
import { line as line$1, curveMonotoneX, curveStep, curveLinear, stack, stackOffsetExpand, stackOffsetDiverging, arc, area, pie, radialArea, curveCardinalClosed, curveLinearClosed, radialLine } from "d3-shape";
...
class LinearAxisTickSeries extends Component {
   ...
   */
  getLabelFormat() {
    const { label: label2, scale: scale2 } = this.props;
   ...
   */
  getTicks() {
    const { scale: scale2, tickSize, tickValues, interval, axis, label: label2 } = this.props;
    const dimension = this.getDimension();
    const maxTicks = getMaxTicks(tickSize, dimension);
    const ticks = getTicks(scale2, tickValues, axis.type, maxTicks, interval);
...
class LinearAxis extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { height, width, scale: scale2 } = this.props;
    if (width !== prevProps.width || height !== prevProps.height || scale2 !== prevProps.scale) {
      this.updateDimensions();
    }
  }
  updateDimensions() {
  ...
  render() {
    ...
    return /* @__PURE__ */ jsxs("g", { transform: `translate(${translateX}, ${translateY})`, ref: this.ref, children: [
...
LinearAxis.defaultProps = {
  ...
  tickSeries: /* @__PURE__ */ jsx(LinearAxisTickSeries, {}),
  scaled: false,
  roundDomains: false,
...
const RadialAxisTickSeries = ({
  ...
  return /* @__PURE__ */ jsx(Fragment, { children: ticks.map((data, i) => /* @__PURE__ */ jsx(
    CloneElement,
    {
      element: tick,
      index: i,
...
const getClosestPoint = (pos, scale2, data, attr = "x") => {
  if (scale2.invert) {
    ...
    if (scale2.mariemkoInvert) {
      ...
      prop = scaleBandInvert(scale2)(pos);
    }
...
class ChartBrush extends Component {
  getBrushOffset() {
    let start2;
    let end2;
    const { disabled, domain, scale: scale2 } = this.props;
    if (!disabled && domain) {
  ...
  onBrushChange(event) {
    const { onBrushChange, scale: scale2, width } = this.props;
    if (onBrushChange) {
      let domain;
      if (event.start !== void 0 && event.end !== void 0 && (event.start !== 0 || event.end !== width)) {
        if (scale2.invert) {
          ...
          domain = [scale2.domain()[start2], scale2.domain()[end2]];
        }
      }
      onBrushChange({
        domain
  ...
  render() {
    const { scale: scale2, height, width, children, ...rest } = this.props;
    return /* @__PURE__ */ jsx(
      Brush,
      {
        ...rest,
...
class TooltipArea extends Component {
  constructor() {
    ...
    this.transformData = memoize((series) => {
      const { inverse: inverse2, isHorizontal } = this.props;
      const result = [];
      if (inverse2) {
        for (const point2 of series) {
  ...
  render() {
    ...
    return /* @__PURE__ */ jsxs(Fragment, { children: [
      disabled && children,
      !disabled && /* @__PURE__ */ jsxs("g", { onMouseLeave: bind(this.onMouseLeave, this), children: [
...
class ChartZoomPan extends Component {
  ...
  render() {
    ...
    const { scale: scale2, x } = this.getOffset();
    return /* @__PURE__ */ jsx(
      ZoomPan,
      {
        ...rest,
...
const PieArcSeries = ({
  ...
  return /* @__PURE__ */ jsx(Fragment, { children: data.map((arcData, index) => /* @__PURE__ */ jsxs(Fragment, { children: [
...
const PieChart = ({
  ...
  return /* @__PURE__ */ jsx(
    ChartContainer,
    {
      ...
      children: ({ chartWidth, chartHeight }) => /* @__PURE__ */ jsx(
        CloneElement,
        {
          element: series,
          data: getData,
...
const MotionBar = ({ custom, transition, arc: arc2, ...rest }) => {
  ...
  useEffect(() => {
    ...
    const unsub = spring.onChange(
      (v) => d.set(arc2({ ...custom.enter, y: interpolator(v) }))
    );
...
const RadialBarSeries = ({
  ...
  return /* @__PURE__ */ jsx(
    CloneElement,
    {
      ...
      children: /* @__PURE__ */ jsx("g", { clipPath: `url(#${id}-path)`, children: data.map(renderBar) })
    }
  );
};
...
const RadialBarChart = ({
  ...
  const getScales = useCallback(
    (preData, innerRadius2, outerRadius) => {
      ...
      const xScale = scaleBand().range([0, 2 * Math.PI]).domain(xDomain);
      const yScale = getRadialYScale(innerRadius2, outerRadius, yDomain);
      return {
        xScale,
        yScale,
...
const StackedRadialGaugeSeries = ({
  ...
  const renderStackedGauges = useCallback(
    ...
    [rAxis, colorScheme, data, endAngle, innerArc, outerArc, scale2, startAngle]
  );
...
const RadialGauge = ({
  ...
  const scale2 = scaleLinear().domain([minValue, maxValue]).range([startAngle, endAngle]);
  return /* @__PURE__ */ jsx(
    ChartContainer,
    {
      id,
      ...
      className,
      containerClassName,
      children: (props) => cloneElement(series, {
        scale: scale2,
        data,
        startAngle,
        endAngle,
...
const MeterColumn = ({
  ...
  const isActive = scale2(index) <= value2;
...
const FunnelChart = ({
  ...
  const getScales = useCallback(({ chartWidth, chartHeight }) => {
    ...
    const xScale = scaleLinear().domain([0, data.length]).range([0, chartWidth]);
    return {
      yScale,
      xScale
    };
  ...
  const renderChart = useCallback(
    ({ id: id2, chartWidth, chartHeight, chartSized }) => {
      ...
      return /* @__PURE__ */ jsxs(Fragment$1, { children: [
        datas.map((d, i) => /* @__PURE__ */ jsx("g", { style: { transform: `translate(0, ${i * halfOffset}px)` }, children: /* @__PURE__ */ jsx(
          CloneElement,
          {
            element: arc2,
            ...d,
