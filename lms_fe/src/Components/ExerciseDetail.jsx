...
const ExerciseDetail = ({ checkTokenExpiration, isTeacher }) => {
    ...
    useEffect(() => {
        const config = {
            ...
            url: `http://localhost:8080/exercise/${eid}`,
            headers: {
    ...
    useEffect(() => {
        if (isTeacher !== true) {
            const config = {
                method: 'get',
                maxBodyLength: Infinity,
                url: `http://localhost:8080/student/getExerciseScore/${eid}`,
                headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('jwt'),
                },
            };
    ...
    const handleOk = () => {
        ...
        form
            ...
            .then((values) => {
                ...
                const updateConfig = {
                    method: 'put',
                    url: `http://localhost:8080/teacher/exercise/update`,
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('jwt'),
    ...
    const handleDelete = () => {
        axios.delete(`http://localhost:8080/teacher/exercise/delete/${eid}`, {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('jwt')
            }
        }).then((response) => {
            ...
            onOk: async () => {
                try {
                    ...
                    const submitWorkConfig = {
                        method: 'post',
                        url: `http://localhost:8080/student/exercise/submit`,
                        headers: {
                            'Authorization': 'Bearer ' + localStorage.getItem('jwt'),
    ...
    const deleteWork = async () => {
        try {
            ...
            const config = {
                method: 'delete',
                url: `http://localhost:8080/student/exercise/delete/${work.id}`,
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('jwt'),
                },
            };
