...
const Lecture = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/lms/student/getCourseList?current_page=0")
      .then((response) => {
        setData(response.data.data); // Lưu trữ dữ liệu lấy từ API vào state
      })
      .catch((error) => {
  ...
  const renderCol = (item) => {
    return (
      <Col key={item.id} span={8} style={{ marginBottom: "40px" }}>
        <Card
          hoverable
          bordered={true}
          style={{
  ...
  return (
    <>
      <h1> Lectures </h1>
      <Row gutter={16}>{data.map((i) => renderCol(i))}</Row>
    </>
  );
};
